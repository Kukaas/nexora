...
import {
  ...
  PaymentStateBadge,
  RequestStatusBadge,
} from "../../../_components/captain-ui";

export const metadata: Metadata = {
  title: "Request · Captain · Barangay Libtangin",
};

/**
 * The captain's read-only view of one request: the full record, the payment
 * and its proof, and who did what when. There are no actions here — preparing
 * and verifying stay with the secretary and treasurer.
 */
export default async function CaptainRequestDetailPage({
  params,
}: {
  ...
  const backHref = `/captain/${id}/requests`;
  ...
  return (
    <div className="flex flex-col gap-6">
      <Link
        href={backHref}
        className="inline-flex w-fit items-center gap-1.5 rounded-2xl text-sm font-medium text-muted-foreground outline-none transition-colors hover:text-foreground focus-visible:ring-3 focus-visible:ring-ring/30 print:hidden"
      >
      ...
      <div className="grid gap-6 lg:grid-cols-3 lg:items-start">
        {/* Summary: sticky on desktop, first on mobile. */}
            ...
            <dl className="flex flex-col gap-2.5 text-sm">
              ...
              {hasPayment && (
                <div className="flex items-center justify-between gap-3">
                  ...
                  <dd>
                    <PaymentStateBadge status={request.status} />
                  </dd>
                </div>
              )}
              <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex flex-col gap-6 lg:order-1 lg:col-span-2">
          <section>
            ...
            <dl className="grid gap-px overflow-hidden rounded-3xl border border-border bg-border text-sm md:grid-cols-2">
              <Row label="Reference">
          ...
          {/* Proof of payment — only when the document charges a fee. */}
          {hasPayment && (
            <section>
              <h2 className="mb-2 text-xs font-medium tracking-wide text-muted-foreground">
                Proof of payment
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    // When the two-column grid has an odd row count, the last row stretches
    // across both tracks so no empty border-colored cell shows through.
    <div className="flex items-start justify-between gap-4 bg-card px-4 py-3 md:odd:last:col-span-2">
