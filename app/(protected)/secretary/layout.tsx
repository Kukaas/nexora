...
import { SecretaryShell } from "./_components/secretary-shell";
...
export default async function SecretaryLayout({
  ...
  const session = await requireRole(UserRoles.SECRETARY);
  const summary = await getRequestSummary();
  ...
  return (
    <SecretaryShell
      ...
      pendingCount={summary.processingCount}
    >
      {children}
    </SecretaryShell>
  );
